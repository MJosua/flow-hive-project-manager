  ...
  getAllDepartments: async (req, res) => {
    ...
    try {
      const [departments] = await dbPMS.promise().execute(`
        ...
        FROM pm_departments d
        LEFT JOIN pm_users u ON d.department_head = u.user_id
        LEFT JOIN pm_projects p ON d.department_id = p.department_id
        LEFT JOIN pm_teams t ON d.department_id = t.department_id
        LEFT JOIN pm_users usr ON d.department_id = usr.department_id
        WHERE d.is_active = 1
  ...
  getDepartmentDetail: async (req, res) => {
    ...
    try {
      ...
      const [teams] = await dbPMS.promise().execute(`
        ...
        FROM pm_teams t
        LEFT JOIN pm_users u ON t.team_leader_id = u.user_id
  ...
  createDepartment: async (req, res) => {
    ...
    try {
      ...
      const [newDepartment] = await dbPMS.promise().execute(
        'SELECT * FROM pm_departments WHERE department_id = ?',
        [data.department_id]
      );
