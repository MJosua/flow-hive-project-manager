...
module.exports = {
  ...
  submitTaskApproval: async (req, res) => {
    ...
    try {
      ...
      // Update task status
      await dbPMS.promise().execute(`
        UPDATE pm_tasks SET status = 'pending_approval' WHERE task_id = ?
      `, [task_id]);
  ...
  submitProjectApproval: async (req, res) => {
    ...
    try {
      ...
      await dbPMS.promise().execute(`
        UPDATE pm_projects SET status = 'pending_approval' WHERE project_id = ?
      `, [project_id]);
    ...
    try {
      ...
      if (approvalInfo.length > 0) {
        ...
        if (action === 'reject') {
          ...
          await dbPMS.promise().execute(`
            UPDATE pm_tasks SET status = 'rejected' WHERE task_id = ?
          `, [taskId]);
        } else {
          ...
          if (pendingApprovals[0].pending_count === 0) {
            ...
            await dbPMS.promise().execute(`
              UPDATE pm_tasks SET status = 'approved' WHERE task_id = ?
            `, [taskId]);
          }
        }
      }
    ...
    try {
      ...
      const [taskApprovals] = await dbPMS.promise().execute(`
        SELECT 
          ...
          tw.submitted_date
        FROM t_task_approvals ta
        JOIN pm_tasks t ON ta.task_id = t.task_id
        JOIN pm_projects p ON t.project_id = p.project_id
        JOIN t_approval_workflows tw ON ta.workflow_id = tw.workflow_id
        JOIN pm_users u ON tw.submitted_by = u.user_id
      ...
      const [projectApprovals] = await dbPMS.promise().execute(`
        ...
        JOIN pm_projects p ON pa.project_id = p.project_id
