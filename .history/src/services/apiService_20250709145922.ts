...
class ApiService {
  ...
  async getUserTeams(departmentId: number) {
    try {
      logger.logApiRequest(`/PM/user/${departmentId}/teams`, 'GET');
      const response = await this.makeRequest('GET', `/PM/user/${departmentId}/teams`);
      logger.logApiSuccess(`/PM/user/${departmentId}/teams`, 'GET', 200, response.data);
      return response.data;
    } catch (error: any) {
      logger.logApiError(`/PM/user/${departmentId}/teams`, 'GET', error, { departmentId });
